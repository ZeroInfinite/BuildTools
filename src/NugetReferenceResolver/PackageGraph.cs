...
namespace NugetReferenceResolver
{
    public class PackageGraph
    {
        ...
        public PackageGraph Closure(string packageId)
        {
        ...
        public PackageGraph Without(string packageId)
        {
        ...
        public static PackageGraph Create(LockFile lck, string targetFrameworkName)
        {
            ...
            var dependencyGroup = FindCompatibleDependencyGroup(lck, parsedFramework);
            ...
            var targetFramework = lck.Targets
            ...
            targetFramework = targetFramework ??
                lck.Targets.FirstOrDefault(t => t.TargetFramework.Equals(chosenFramework) &&
                    RuntimeIsCompatible(t.RuntimeIdentifier, runtimeIdentifier, fallbacks));

            targetFramework = targetFramework ?? lck.Targets.FirstOrDefault(t => t.TargetFramework.Equals(chosenFramework) &&
            ...
            var sources = lck.PackageFolders.Select(p => p.Path).ToArray();

            var packages = directDependencies
                .Select(dd => CreatePackage(dd, targetFramework, lck.Libraries, allPackages, sources)).ToArray();

            return new PackageGraph(sources, allPackages, packages, chosenFramework.GetShortFolderName());
        }

        private static ProjectFileDependencyGroup FindCompatibleDependencyGroup(LockFile lck, NuGetFramework parsedFramework)
        {
            return lck.ProjectFileDependencyGroups.FirstOrDefault(g => NuGetFramework.Parse(g.FrameworkName).Equals(parsedFramework)) ??
                lck.ProjectFileDependencyGroups.FirstOrDefault(g => IsCompatible(parsedFramework, NuGetFramework.Parse(g.FrameworkName)));
        }
