    ...
    public class GenerateResxDesignerFiles : Task
    {
        ...
        public override bool Execute()
        {
            foreach (var item in ResourceFiles)
            {
                ...
                {
                    Log.LogMessage(MessageImportance.Low, "Skipping resource '{0}' with type '{1}", item.ItemSpec, item.GetMetadata(Metadata.Type));
                    continue;
                }
    ...
    {{
        private static readonly ResourceManager _resourceManager
            = new ResourceManager(""{manifestName}"", typeof({resourceTypeName}).GetTypeInfo().Assembly);");
                ...
                {
                    writer.WriteLine();
                    RenderHeader(writer, resourceString);
                    RenderProperty(writer, resourceString);

                    writer.WriteLine();
                    RenderHeader(writer, resourceString);
                    RenderFormatMethod(writer, resourceString);
                }
        ...
        private static void RenderProperty(TextWriter writer, ResourceData resourceString)
        {
            ...
            writer.WriteLine("        {");
            writer.WriteLine($@"            get => GetString(""{resourceString.Name}"");");
            writer.WriteLine("        }");
        }
        ...
        private static void RenderFormatMethod(TextWriter writer, ResourceData resourceString)
        {
            ...
            {
                writer.WriteLine(@"            => string.Format(CultureInfo.CurrentCulture, GetString(""{0}""{1}), {2});",
                            resourceString.Name,
            ...
            {
                writer.WriteLine($@"            => GetString(""{resourceString.Name}"");");
            }
        }
